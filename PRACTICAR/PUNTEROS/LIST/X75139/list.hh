...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    if (itemsup.prev != it.pitem) {
      Item* mover = it.pitem;
      Item* siguiente = mover->next;

      mover->prev->next = siguiente;    // no pierdo nada
      siguiente->next->prev = mover;    // no pierdo nada

      mover->next = siguiente->next;    // da igual porque (mover->next = siguiente)
      siguiente->prev = mover->prev;    // da igual porque (siguiente->prev = mover)
