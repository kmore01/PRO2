  ...
  class iterator {
       ...
       el resultat és el p.i. */
    {
        if (not isHoock) {
            ...
            if (pitem == &(plist->itemsup) or pitem->next == &(plist->itemsup)) {
                return *this;
            }
            Item* mover = pitem;
            Item* siguiente = mover->next;

            siguiente->prev = mover->prev;      // MOVER
            mover->next = siguiente->next;      // SIGUIENTE

            mover->prev->next = siguiente;
            siguiente->next->prev = mover;

            siguiente->next = mover;
            mover->prev = siguiente;
       ...
       el resultat és el p.i. */
    {
        if (not isHoock) {
            ...
            Item* mover = pitem;
            Item* anterior = pitem->prev;

            anterior->next = mover->next;
            mover->prev = anterior->prev;

            anterior->prev->next = mover;
            mover->next->prev = anterior;
