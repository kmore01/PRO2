...
public:
  ...
  void swapFirstLast() {
    if (_size > 1) {
      ...
      primero->prev = ultimo->prev;     // no pierdes nada (primero->prev = &iteminf)
      ultimo->next = primero->next;     // no pierdes nada (utlimo->next = &itemsup)

      primero->next->prev = ultimo;
      ultimo->prev->next = primero;
