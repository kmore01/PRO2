...
private:
  ...
  static bool evaluate_aux(node_arbre *n ) {
    ...
    // Caso general
    bool izq = evaluate_aux(n->segE);
    bool der = evaluate_aux(n->segD);
    if (n->info == "and") return izq and der;
    else if (n->info == "or") return izq or der;
    else if (n->info == "not") return not izq;
