  ...
  class iterator {
       ...
       el resultat és el p.i. */
    {
      if (pitem != &(plist->itemsup)) {
        if (not activateHook) pitem = pitem->next;
        else {
          if (pitem->next != &(plist->itemsup)) {
            Item *previo = pitem->next;
            Item *siguiente = pitem->next->next;

            siguiente->prev = pitem;  
            
            //siguiente->next = pitem->next->next->next;

            previo->prev = pitem->prev;
            previo->next = pitem;

            pitem->prev->next = previo;
            //pitem->next->next->next->prev = siguiente;

            pitem->next = siguiente;
            pitem->prev = previo;
       ...
       el resultat és el p.i. */
    {
      ...
      if (activateHook) { // swap
        
          Item *previo = pitem->prev->prev;
          Item *siguiente = pitem->prev;

          siguiente->next = pitem->next;
          siguiente->prev = pitem;

          previo->next = pitem;
          //previo->prev = pitem->prev->prev->prev;

          //pitem->prev->prev->prev->next = previo;
          pitem->next->prev = siguiente;
