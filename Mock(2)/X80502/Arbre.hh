...
Arbre suma (const Arbre& a) const {
  node_arbre* res = sum_aux(primer_node, a.primer_node);
  Arbre resultat;
  resultat.primer_node = copia_node_arbre(res);
  return resultat;
}
private:
...
static node_arbre* sum_aux(node_arbre* n,  node_arbre *m) {
  ...
  // Caso general
  node_arbre* izq = sum_aux(n->segE, m->segE);
  node_arbre* der = sum_aux(n->segD, m->segD);
  
  node_arbre* res = new node_arbre;
  res->info = n->info + m->info;
  res->segE = izq;
  res->segD = der;
  return res;
}  
